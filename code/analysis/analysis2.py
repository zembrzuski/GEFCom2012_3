...
year_month_predicate = cool_functions.year_month_predicate(2004, 1)
zone_predicate = cool_functions.zone_predicate(1)
data = list(filter(lambda x: zone_predicate(x), filter(lambda x: year_month_predicate(x), only_available_data)))
